...
export class RateLimiterInterceptor implements NestInterceptor {
    private logger = new AppLogger(RateLimiterInterceptor.name);
    ...
    async getRateLimiter(keyPrefix: string, options?: RateLimiterModuleOptions): Promise<RateLimiterMemory> {
        ...
        if (!rateLimiter) {
            if (limiterOptions.type === 'Memory') {
                ...
                this.logger.log(`Created RateLimiterPostgres with keyPrefix = ${keyPrefix}`);
                ...
                this.logger.log('Created RateLimiterMySQL with keyPrefix =', keyPrefix);
            } else {
                throw new Error(
